...
function googleSearch(msg, client, query, qtd, image){
	let id = process.env.GOOGLE_API_CUSTOM_SEARCH_ID
	let key = process.env.GOOGLE_API_KEY
	let num = qtd || 1
	let searchType = image ? '&searchType=image' : '';

	try{
		const searchParams = {
			method: 'GET',
			uri: `https://customsearch.googleapis.com/customsearch/v1?cx=${id}&num=${num}&key=${key}${searchType}&q=${query}`
		};

		rp(searchParams).then(response =>{
			let result = JSON.parse(response)
			if(result && result.items && result.items.length > 0){
				result.items.forEach((it, idx) =>{
					let resultNumber = idx+1;
					let struct = image ? {
						title: it.title,
						image: it.link,
						pageLink: it.image && it.image.contextLink
					} : 
					{
						title: it.title,
						pageLink: it.link,
						snippet: it.snippet,
						image: it.pagemap && it.pagemap.cse_thumbnail && it.pagemap.cse_thumbnail[0] && it.pagemap.cse_thumbnail[0].src
					}
					...
					try{
						if(struct.image){
							if(image){
								let text = "Resultado "+resultNumber 
								text += '\n\nContexto:'
								text += '\n'+struct.title
								text += '\n'+struct.pageLink
								download(struct.image, client, msg, text)
							}else{
								let text = "Resultado "+resultNumber 
								text += '\n'+struct.title
								text += '\n'+struct.pageLink
							...
							text += '\n'+struct.pageLink
							text += '\n\n'+struct.snippet
							client.sendMessage(msg.from, text, {quotedMessageId: msg.id._serialized})
						}
...
function getGoogleParams(body){
	var comando = body.slice(0,7)
	var texto = body.slice(7)
  
	//se tiver |, usar pra pegar os modifiers
	if(body.indexOf('|')>0){
	  texto = body.slice(7, body.indexOf('|'))
	  var modifiers = +body.slice(body.indexOf('|')+1).split(' ').filter(m => m)
	}
...
function getPartPics(mentions, chat){
	let participants = mentions.map(ment => ment.id.user)
	let num1 = participants[0]
	let num2 = participants[1]
    let pic
	...
	if(groupSettings && groupSettings.groups){
		// procura as configuraçoes do grupo em questao
		let group = groupSettings.groups.find(group => group.id === chat.id.user);
		if(group && group.groupPhotos && group.groupPhotos.length){
			let partPics = group.groupPhotos.find(pic => pic.part === num1)
    
			if(partPics && partPics.photos && partPics.photos.length>0){
				let pics = partPics.photos.find(pics => pics.par === num2)
				if(pics && pics.photos && pics.photos.length > 0){
					pic = pics.photos[getRandomInt(pics.photos.length)]
				}
			}
		}
	}
